...
export async function deployTestHook(): Promise<Contract> {
    const TestHook = await locklift.factory.getContract('TestHook');
    const [keyPair] = await locklift.keys.getKeyPairs();
    const _randomNonce = locklift.utils.getRandomNonce();

    const testHook = await locklift.giver.deployContract({
        contract: TestHook,
        constructorParams: {},
        initParams: {
            _randomNonce,
    ...
    testHook.setKeyPair(keyPair);
    testHook.afterRun = afterRun;
    testHook.name = 'Test Hook';
