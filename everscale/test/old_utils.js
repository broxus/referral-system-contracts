...
const setupBridge = async (relays) => {
  ...
  const CellEncoder = await locklift.factory.getContract('HookedProxyMultiVaultCellEncoder');
...
const setupAlienMultiVault = async (owner, staking, cellEncoder) => {
  ...
  // Deploy proxy
  const Proxy = await locklift.factory.getContract('HookedProxyMultiVaultAlien_V3');
  const proxy = await locklift.giver.deployContract({
    contract: Proxy,
    constructorParams: {
      owner_: owner.address,
  ...
  const EthereumEventConfiguration = await locklift.factory.getContract('EthereumEventConfiguration');
  const EthereumEvent = await locklift.factory.getContract('HookedMultiVaultEVMEventAlien');
  ...
  const EverscaleEventConfiguration = await locklift.factory.getContract('EverscaleEventConfiguration');
  const EverscaleEvent = await locklift.factory.getContract('HookedMultiVaultEverscaleEventAlien');
