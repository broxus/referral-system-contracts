...
describe('Deposit Alien token from EVM to Everscale with no merging', async function () {
    ...
    describe('Confirm event', async () => {
        ...
        it('Check RefFactory received Event', async () => {
            const receieved = await refFactory.call({
                method: 'DEBUG_RECV_COUNT',
                answerId: 0
            })
            const parents = await refFactory.call({
                method: 'DEBUG_RECV_PARENTS',
                answerId: 0
            })
