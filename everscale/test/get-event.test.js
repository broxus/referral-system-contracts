...
const { locklift, afterRun, deployTestHook, logContract } = require('./utils')
...
describe('Deposit Alien token from EVM to Everscale with no merging', async function() {
  ...
  it('Setup bridge', async () => {
      ...
      testHook = await deployTestHook()
      logContract(testHook)
  });
  ...
  describe('Confirm event', async () => {
      ...
      it('Check TestHook received Event', async () => {
        const receieved = await testHook.call({
          method: 'recievedEvents',
          answerId: 0
        })

        expect(receieved)
          .to.be.bignumber.greaterThan(0, 'TestHook did not receive Event')
      })
  });
});
